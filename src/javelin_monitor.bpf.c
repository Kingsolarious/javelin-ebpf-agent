 ...
 *
 * Erick wrote this. Nick debugged the verifier rejections.
 * Dyllan keeps asking if we can add a GUI. The answer is still no.
 *
...
/* LSM: file_mprotect */
/* NOTE: this fires on EVERY mprotect call for target PIDs. In a game
 * this could be noisy. We might need to rate-limit or batch these.
 * TODO: nick says we should add a per-PID throttle map. */
...
/* NOTE: this is janky. We do a partial string match because full strcmp
 * in eBPF is expensive. Nick says it's fine because userland can filter
 * false positives. Dyllan thinks we should do a proper bloom filter.
 * Both are wrong. We should just match on the fd after open. FIXME. */
